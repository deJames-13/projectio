    ...
    <div id="docs-view" className="p-8 max-w-7xl mx-auto space-y-6 animate-in fade-in duration-200">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-3 pb-2 border-b border-slate-200">
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto w-full md:w-auto pb-1 md:pb-0 scrollbar-none">
          ...
          >
            ...
            <span className="ml-1 text-[10px] px-1.5 py-0.2 rounded-full bg-slate-100 text-slate-700">
              {docs.length}
            </span>
          </button>
        ...
        <div className="relative w-full md:w-56">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-1.5 bg-white border border-slate-200 rounded-lg text-xs text-slate-900 placeholder:text-slate-400 focus:border-blue-500 focus:outline-none"
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6 min-h-[550px]">
        ...
        <div className="md:col-span-4 space-y-2">
          <div className="bg-white rounded-xl p-2 border border-slate-200 shadow-xs space-y-1 max-h-[600px] overflow-y-auto">
            {filteredDocs.map((d) => {
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5">
                      ...
                      {isPersonal && (
                        <span className="inline-flex items-center gap-0.5 px-1.5 py-0.2 rounded bg-amber-50 text-amber-700 border border-amber-200 text-[9px] font-medium" title="Personal note">
            ...
            {filteredDocs.length === 0 && (
              <div className="p-8 text-center space-y-2">
                ...
                <p className="text-xs font-medium text-slate-500">No documents found</p>
        ...
        <div className="md:col-span-8 bg-white rounded-xl p-6 border border-slate-200 shadow-xs flex flex-col justify-between">
          {currentDoc ? (
            <div className="space-y-5">
                ...
                <div className="flex-1 mr-4">
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    {currentDoc.projectId ? (
                      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-blue-50 text-blue-700 border border-blue-200 text-xs font-medium">
                        <Folder className="w-3 h-3" />
                        <span>Project: {currentDoc.projectName ?? projects.find((p) => p.id === currentDoc.projectId)?.title ?? 'Linked'}</span>
                      </span>
                    ) : (
                      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-amber-50 text-amber-700 border border-amber-200 text-xs font-medium">
                        <Lock className="w-3 h-3" />
                        <span>Personal Document (Confidential)</span>
                      </span>
                    )}

                    <span className="text-xs text-slate-400">• {currentDoc.category}</span>
                  </div>
                  ...
                  {isEditing ? (
                    <div className="space-y-3">
                      <input
                        ...
                        className="text-xl font-bold text-slate-900 w-full border-b border-blue-500 pb-1 focus:outline-none bg-transparent"
                      ...
                      <div className="flex items-center gap-2 text-xs">
                        ...
                        <select
                          ...
                          className="bg-slate-50 border border-slate-200 rounded px-2 py-1 text-xs text-slate-800 cursor-pointer"
                        >
                ...
                <div className="flex items-center gap-2">
                  {isEditing ? (
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 bg-slate-50 hover:bg-slate-100 text-slate-700 border border-slate-200 rounded-lg text-xs font-medium flex items-center gap-1.5 transition-colors cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"
                    >
              ...
              {isEditing ? (
                <textarea
                  ...
                  className="w-full bg-slate-50 border border-slate-200 focus:border-blue-500 focus:bg-white focus:outline-none rounded-lg p-4 text-xs font-mono text-slate-800 leading-relaxed resize-none"
                />
              ) : (
                <div className="prose max-w-none text-xs text-slate-800 leading-relaxed whitespace-pre-wrap">
                  {currentDoc.content}
                </div>
              )}
            </div>
            ...
            <div className="p-12 text-center text-slate-400 flex flex-col items-center justify-center space-y-2">
              ...
              <p className="text-sm font-semibold text-slate-700">Select a document to read</p>
              <p className="text-xs text-slate-400">Choose a document from the left list or create a new one.</p>
            </div>
          )}
        </div>
      </div>
          ...
          <div className="w-full max-w-lg bg-white rounded-xl p-6 shadow-xl border border-slate-200 animate-in zoom-in-95 duration-150">
            ...
            <form onSubmit={handleCreateDocSubmit} className="space-y-4 mt-4">
              ...
              <p className="text-[11px] text-slate-500 bg-slate-50 p-2.5 rounded-lg border border-slate-200">
                {newProjectId !== '' ? (
                  <span>
                    👥 <strong className="text-slate-700">Shared Project Doc:</strong> All members of this project can view and collaborate on this specification.
                  </span>
                ) : (
                  <span>
                    🔒 <strong className="text-slate-700">Personal Note:</strong> Only your account will have access to view or edit this document.
                  </span>
                )}
              </p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-xs" role="dialog" aria-modal="true">
          <div className="w-full max-w-md bg-white rounded-xl p-6 shadow-xl border border-rose-100 animate-in zoom-in-95 duration-150">
            ...
            <p className="text-xs text-slate-500 mt-1.5 leading-relaxed">
              Are you sure you want to permanently delete <span className="font-semibold text-slate-900">&quot;{deletingDoc.title}&quot;</span>? This action cannot be undone.
            </p>
